...
from minesweeping import blockCanvas
import random
...
def initMap(rootframe):
    ...
    :return: 棋盘画布
    """
    frameWidth = MAP_WIDTH*30 + 40
    frameHeight = MAP_WIDTH*30 + 90
    ...
    for i in range(0, MAP_WIDTH):
        map.create_line(20, 70+i*30, frameWidth-20, 70+i*30) # 横线
        map.create_line(20+i*30, 70, 20+i*30, frameHeight-20) # 竖线
...
def drawDigitsOnSide(canvas):
    ...
    """
    for i in range(0, MAP_WIDTH):
        canvas.create_text(10, 95+i*30, text=str(i))
        canvas.create_text(45+i*30, 60, text=str(i))
    ...
    for i in range(0, BOX_NUM):
        if gameData[str(i)] == -1:
            if i == No:
                px = Blocks[str(i)].x
                py = Blocks[str(i)].y
                gameMap.create_rectangle(px, py, px+30, py+30, fill='red')
